...
describe("Account services", () => {
    ...
    let userResetInput : UpdateUser = {
        ...
        lastName: "PAPP",
        middleName: " ",
        picURL: "https://lh3.googleusercontent.com/a/ALm5wu0YMGcog8kBVUkqqB39RgeOY8oiG0QgIJzqddPfYA=s96-c",
    ...
    afterAll(async () => {
        ...
        await resolver.update({...userResetInput, instructorProfile: updatedInstructorProfileReset})
    ...
    describe("Instructor", () => {
        ...
        it('should update instructor profile given input argument', async () => {
            let updatedInstructorProfile : InstructorProfileInput = {
                ...
                contactPolicy: "Email",
                phone: "111-222-3333",
                background: "I'm a very experienced professor, and I only use books from 50 years ago.",
            ...
            expect(updateUser.instructorProfile.title).toEqual(updatedInstructorProfile.title);
            expect(updateUser.instructorProfile.officeLocation).toEqual(updatedInstructorProfile.officeLocation);
            expect(updateUser.instructorProfile.officeHours).toEqual(updatedInstructorProfile.officeHours);
            expect(updateUser.instructorProfile.contactPolicy).toEqual(updatedInstructorProfile.contactPolicy);
            expect(updateUser.instructorProfile.phone).toEqual(updatedInstructorProfile.phone);
            expect(updateUser.instructorProfile.background).toEqual(updatedInstructorProfile.background);
            expect(updateUser.instructorProfile.researchInterest).toEqual(updatedInstructorProfile.researchInterest);
            expect(updateUser.instructorProfile.selectedPapersAndPublications).toEqual(updatedInstructorProfile.selectedPapersAndPublications);
            expect(updateUser.instructorProfile.personalWebsite).toEqual(updatedInstructorProfile.personalWebsite);
            expect(updateUser.instructorProfile.philosophy).toEqual(updatedInstructorProfile.philosophy);
        });
    })
    describe("Social", () => {
        ...
        it('should update social profile given input argument', async () => {
            ...
            const updatedSocialDoc = await resolver.updateSocial(socialDocumentID, updatedSocial)
